...
/*  Selecting all the feedback containers   */
...
const validate = (submitBtnClicked) => {
  ...
  /*  Checking if the form is submitted at least once to avoid initial
  ...
  if (submittedOnce) {
    ...
    if(!mail.startsWith('@') && mail.includes('@') && mail.includes('.') && mail.length - mail.lastIndexOf('.') >= 3){
    ...
    if(password.length >= 8 && ['$', '#', '@'].some(val => password.includes(val)) && ['1', '2', '3', '4'].some(val => password.includes(val))){
